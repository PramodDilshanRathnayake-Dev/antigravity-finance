...
public class CapitalConstraintTest {
    ...
    void testConstraint_WhenNoProfit_TradeDenied() {
        // Arrange
        String userId = "user1";
        Portfolio portfolio = new Portfolio(userId, new BigDecimal("10000.00")); // Initial Capital = 10,000, Profit = 0
    ...
    void testConstraint_WhenRiskExceeds10PercentProfit_TradeDenied() {
        ...
        // Assert
        assertFalse(response.approved());
        assertTrue(response.message().contains("DENIED: Risk (300.00) exceeds 10% CVaR threshold"));
